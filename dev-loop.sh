...
review_prs() {
    ...
    while IFS='|' read -r pr_num _updated_at pr_title branch_name; do
        ...
        if [ -n "$feedback" ]; then
            ...
            if [ -n "$linked_issue_num" ]; then
                prompt+="You are working on PR #${pr_num} titled \"${pr_title}\" which addresses issue #${linked_issue_num}.

Issue #${linked_issue_num} \"${linked_issue_title}\":
...
"
                ...
                prompt+="You are working on PR #${pr_num} titled \"${pr_title}\".
"
            fi

            prompt+="Original PR description:
${pr_body}
...
If anything is ambiguous, print your questions to stdout prefixed with \`QUESTION:\` so the wrapper script can post them."
            ...
            echo "[debug]   Updating PR description for #${pr_num}" >&2
            local current_diff
            current_diff="$(git diff "${DEFAULT_BRANCH}..origin/${branch_name}")" || current_diff=""
            local current_log
            current_log="$(git log --oneline "${DEFAULT_BRANCH}..origin/${branch_name}")" || current_log=""

            local update_summary_prompt="Generate a concise markdown summary of the following git changes. Include:
...
${current_diff}"
            ...
            local new_pr_body=""
            if [ -n "$linked_issue_num" ]; then
                new_pr_body+="Closes #${linked_issue_num}.
"
            fi
            new_pr_body+="
${updated_summary}
...
*Autogenerated by busybee-bot.*"
            ...
            gh pr edit "$pr_num" --repo "$REPO" --body "$new_pr_body" >/dev/null 2>&1 || true
